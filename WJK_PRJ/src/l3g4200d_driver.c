...
u8_t ReadReg(u8_t Reg,u8_t *Data) {
  ...
  a99 = i2c_read_reg(L3G4200_I2C_Moudle,I_AM_L3G4200D, Reg);
...
u8_t WriteReg(u8_t Reg, u8_t Data) {
  ...
  i2c_write_reg(L3G4200_I2C_Moudle,I_AM_L3G4200D, Reg, Data);
...
status_t SetODR(ODR_t ov){
  ...
  if( !ReadReg(L3G4200_CTRL_REG1, &value) )
    return MEMS_ERROR;

  value &= 0x0f;
  value |= ov<<4;

  if( !WriteReg(L3G4200_CTRL_REG1, value) )
    return MEMS_ERROR;
...
status_t SetFullScale(Fullscale_t fs) {
  ...
  if( !ReadReg(L3G4200_CTRL_REG4, &value) )
    return MEMS_ERROR;
                  
  value &= 0xCF;	
  value |= (fs<<FS);
  
  if( !WriteReg(L3G4200_CTRL_REG4, value) )
    return MEMS_ERROR;
...
status_t HPFEnable(State_t hpf) {
  ...
  if( !ReadReg(L3G4200_CTRL_REG5, &value) )
    return MEMS_ERROR;
                  
  value &= 0xEF;
  value |= (hpf<<HPEN);
  
  if( !WriteReg(L3G4200_CTRL_REG5, value) )
    return MEMS_ERROR;
...
status_t SetHPFMode(HPFMode_t hpf) {
  ...
  if( !ReadReg(L3G4200_CTRL_REG2, &value) )
    return MEMS_ERROR;
                  
  value &= 0xCF;
  value |= (hpf<<HPM);
  
  if( !WriteReg(L3G4200_CTRL_REG2, value) )
    return MEMS_ERROR;
...
status_t SetIntPinMode(IntPinMode_t pm) {
  ...
  if( !ReadReg(L3G4200_CTRL_REG3, &value) )
    return MEMS_ERROR;
                  
  value &= 0xEF;
  value |= (pm<<PP_OD);
  
  if( !WriteReg(L3G4200_CTRL_REG3, value) )
    return MEMS_ERROR;
...
void L3G4200_Init(void)
{
  ...
  SetODR(ODR_800Hz_BW_30);
    //enable all axis  
  SetAxis(X_ENABLE | Y_ENABLE | Z_ENABLE); 
    //set the fullscale
  SetFullScale(FULLSCALE_500);
    //set sensor mode
  SetMode(NORMAL);
    //set the fifo mode
